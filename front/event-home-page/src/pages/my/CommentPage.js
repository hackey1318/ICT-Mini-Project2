...
function CommentsPage() {
    ...
    async function getReplyList() {
        try {
            ...
            console.error("후기 목록 가져오기 실패:", error);
    ...
    const handleDelete = async (replyNo) => {
        if (window.confirm("후기을 삭제하시겠습니까?")) {
    ...
    return (
        <div className="container">
            <h3 className="mb-4 d-none d-md-block">작성한 후기</h3>
            <div style={{ display: "flex" }}>
                ...
                <div className="right" style={{ flex: 1, padding: "30px" }}>
                    <div className="row" style={{ borderBottom: 'solid #ddd 2px' }}>
                        ...
                        <div className="col-sm-4 p-2">후기 내용</div>
                        <div className="col-sm-2 p-2">작성 날짜</div>
                        <div className="col-sm-2 p-2">수정 / 삭제</div>
                    </div>
                    {replyData.map(function (record) {
                        const isDeleted = record.status === "DELETE";
                        return (
                            <div className="row" style={{
                                borderBottom: 'solid #ddd 2px',
                                backgroundColor: isDeleted ? '#f0f0f0' : 'transparent' // DELETE 상태면 회색으로 배경 설정
                            }} key={record.no}>
